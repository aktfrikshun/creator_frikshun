...
The daily-post editor also provides an **Archive classification** workflow. A reviewed post can be categorized by topic and pushed as a daily signal, field note, recovery log, collaboration record, context note, or canon proposal. Every durable record carries an explicit evidence/canon status and provenance. Generation eligibility is a separate opt-in and is accepted only for confirmed canon or an accepted model; generated artifacts, drafts, contradictions, and unresolved mysteries cannot silently enter generation context. Repeating a push for the same post and record type updates the stable Markdown record instead of creating duplicates.
